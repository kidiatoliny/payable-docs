...
map to HTTP statuses through `src/presentation/shared/payable-http.ts`.
...
- Raw body was consumed by a JSON parser. On Express, a global `express.json()` mounted before the
  ...
  `express.raw(...)` for the webhook routes (`src/presentation/express/routes/webhooks.routes.ts`).
- NestJS app not bootstrapped with `rawBody: true`. The controller reads `request.rawBody`; without
  ...
  `NestFactory.create(AppModule, { rawBody: true })` (`docs/adapters/24-nestjs.md`).
...
segment). `defaultWebhookProvider()` throws when `registry.names().length > 1` (`src/payable.ts`).
...
Cause: an operation was requested against a provider that does not implement that capability
(`src/domain/errors/provider-capability-not-supported.error.ts`). The error `context` carries
`{ provider, capability }`.
...
- `IDEMPOTENCY_CONFLICT`: the same idempotency key was reused with a different request payload
  (`src/domain/errors/idempotency-conflict.error.ts`; `context.key`).
...
(`src/infrastructure/encryption/node-encryption-driver.ts`).
...
`storage` driver was configured (`src/payable.ts` - `webhookDependencies` and `outbox` both check
`this.resolved.storage`).
...
- Redis is unreachable or misconfigured. `BullMQQueueDriver` needs a valid `connection`
  (`src/infrastructure/queue/bullmq/bullmq-queue-driver.ts`); confirm Redis is up and the
  connection options are correct.
...
501 placeholders on all three adapters (`src/presentation/fastify/routes/placeholder.routes.ts`,
`src/presentation/nest/payable.controller.ts`).
...
Resolution: run `migrate(knex)` against the exact connection passed to `KnexStorageDriver`
(`src/infrastructure/storage/knex/migrations/migrate.ts`). It creates billing and system tables and
applies additive alters; it is safe to re-run.
