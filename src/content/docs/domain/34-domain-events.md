---
title: "Domain Events"
description: "Domain events live in src/domain/events/. Each event is an immutable record of something that happened in the domain. Each extends DomainEvent and carries a..."
sidebar:
  order: 34
---
...
The event classes are exported public API (typed contracts consumers can build, dispatch, and subscribe to via the [`EventBus`](/domain/33-contracts/#eventbus); default `InMemoryEventBus`). Two emission paths exist, and they are distinct - read this before assuming an event fires:
...
Monetary fields on payloads carry [`Money`](/domain/06-value-objects/#money) (for example `amount` / `total`), not raw integers. See each `*.event.ts` file for the full payload definitions.
