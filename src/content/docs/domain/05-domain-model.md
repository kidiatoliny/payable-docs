...
| `StoredMoney` | `amount: number`, `currency: CurrencyCode` | Persisted money shape (minor units + currency code). See [Value Objects](/domain/06-value-objects/) for the `Money` behavior. |
...
Monetary amounts on entities (`total`, `amountPaid`, `amountDue`, `amount`, `unitAmount`, `refundedAmount`) are plain `number` values expressed in **minor units** (cents for two-decimal currencies). They are never floats representing major units. The `currency: CurrencyCode` field on the same entity tells you how to interpret them. See [Value Objects](/domain/06-value-objects/) for the no-floats rule and the `Money` helper that wraps these stored amounts.
...
| `status` | `SubscriptionStatus` | One of the values in [`subscription-status`](/domain/06-value-objects/#subscription-status). |
...
Lifecycle: `status` is governed by the [Subscription state machine](/domain/07-state-machines/#subscription). The date fields (`trialEndsAt`, `endsAt`) drive the lifecycle predicates below.
...
| `status` | `InvoiceStatus` | One of the values in [`invoice-status`](/domain/06-value-objects/#invoice-status). |
...
Lifecycle: `status` is governed by the [Invoice state machine](/domain/07-state-machines/#invoice). Amount fields are minor-unit integers interpreted by `currency`.
...
| `status` | `PaymentStatus` | One of the values in [`payment-status`](/domain/06-value-objects/#payment-status). |
| `currency` | `CurrencyCode` | Currency of the amounts below. |
...
Lifecycle: `status` is governed by the [Payment state machine](/domain/07-state-machines/#payment). `refundedAmount` tracks cumulative refunds; the `partially_refunded` and `refunded` payment states correspond to partial vs. full refunds.
...
| `status` | `RefundStatus` | One of the values in [`refund-status`](/domain/06-value-objects/#refund-status). |
...
Lifecycle: `status` is governed by the [Refund state machine](/domain/07-state-machines/#refund).
...
Invariants (enforced outside the entity): `(provider, providerEventId)` uniquely identifies an event, supporting idempotent webhook handling. See [Value Objects](/domain/06-value-objects/#idempotencykey) for `IdempotencyKey.forWebhook`.
...
| --- | --- | --- |
| `id` | `string` | Local identifier. |
| `url` | `string` | Delivery target URL. See [`WebhookEndpointUrl`](/domain/06-value-objects/#webhookendpointurl) for the HTTPS-only, non-routable-host validation applied on input. |
| `events` | `readonly string[]` | Normalized event types this endpoint subscribes to. |
| `secret` | `string` | Signing secret used to sign deliveries. See [`WebhookSigningSecret`](/domain/06-value-objects/#webhooksigningsecret). |
...
The HTTP status each `code` maps to (for example `IDEMPOTENCY_CONFLICT` -> 409, `PROVIDER_CAPABILITY_NOT_SUPPORTED` -> 422) is documented in [Troubleshooting](/31-troubleshooting/).

## DTOs

The input/output shapes in `src/domain/dtos/` are the **boundary types**: the plain interfaces that builders and actions accept and return, and that the [`PaymentProvider`](/domain/33-contracts/#paymentprovider) contract is defined in terms of. Monetary fields on these DTOs use the [`Money`](/domain/06-value-objects/#money) value object (not raw integers), unlike the persisted entity shapes above. They are re-exported from `src/domain/dtos/index.ts`.
