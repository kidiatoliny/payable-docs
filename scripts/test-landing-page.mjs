...
assert.equal(lifecycle.match(/class="code-line"/g)?.length, 7);
assert.match(lifecycle, /items: \[/);
assert.match(lifecycle, /priceId: <span class="str">'price_pro'<\/span>, quantity: 1/);
...
assert.match(styles, /\.dark-code\s*\{[^}]*border-radius:\s*26px[^}]*background:\s*#fff[^}]*box-shadow:/s);
assert.match(styles, /\.dark-code::before\s*\{/);
assert.match(styles, /\.dark-code::after\s*\{/);
assert.match(styles, /\.dark-code \.line-no\s*\{[^}]*flex:\s*0 0 38px[^}]*text-align:\s*right/s);
assert.match(styles, /\.dark \.dark-code\s*\{[^}]*background:\s*#1c1c21/s);
