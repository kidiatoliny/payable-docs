...
assert.doesNotMatch(layout, /astro:before-preparation|activeElement\.blur/);
assert.match(layout, /<Header nav=\{nav\} currentSlug=\{slug\} \/>/);
assert.match(layout, /<Sidebar nav=\{nav\} currentSlug=\{slug\} \/>/);
assert.doesNotMatch(layout, /<html[^>]*class="dark"/);
assert.match(layout, /globals\.css\?url/);
assert.match(layout, /docs\.css\?url/);
assert.match(layout, /transition:persist="global-styles"/);
assert.match(layout, /transition:persist="docs-styles"/);
assert.match(layout, /class="docs-page/);
assert.match(layout, /class="docs-shell/);
assert.match(header, /sticky top-0 z-40/);
assert.match(header, /<header[^>]*transition:persist="docs-header"/s);
...
assert.match(mobileNav, /<Sidebar side="left">/);
assert.match(mobileNav, /SidebarTrigger/);
assert.doesNotMatch(mobileNav, /SheetContent/);
